...
export class InputFlag
{
    constructor(element)
    {
        ...
        // If element is null, create a dummy object to prevent errors
        if(!this.element)
        {
        ...
        if(countryCode !== '' && this.currentElement && this.buttonElement)
        {
    ...
    setSearch()
    {
        if(!this.searchElement || !this.noResultElement)
            return
    ...
    {
        // Already
        if(this.isOpen || !this.selectElement || !this.searchElement)
            return
        ...
        if(this.country && this.scrollerElement)
