...
export class CircuitArea extends Area
{
    ...
    constructor(model)
    {
        ...
        this.setAirDancers()
        this.setBanners()
        this.setData() // Initialize local scores first
        this.setMenu()
        this.setEndModal()
        this.setLeaderboard()
        this.setResetTime()
    ...
    setBanners()
    {
        ...
        // Creative animated banners to replace Three.js flags
        // Create colorful gradient materials with animated patterns
        const bannerColors = [
            { primary: '#FF6B6B', secondary: '#4ECDC4' }, // Coral to Teal
            { primary: '#A855F7', secondary: '#EC4899' }, // Purple to Pink
            { primary: '#F59E0B', secondary: '#EF4444' }, // Amber to Red
        ]
        
        let colorIndex = 0
        for(const banner of this.banners)
        {
            const colors = bannerColors[colorIndex % bannerColors.length]
            
            // Create animated gradient material for each banner
            const bannerMaterial = banner.material.clone()
            
            const baseOutput = bannerMaterial.outputNode
            
            bannerMaterial.outputNode = Fn(() =>
            {
                const baseUv = uv()
                const time = this.game.ticker.elapsedScaledUniform
                
                // Animated wave pattern
                const wave = sin(baseUv.y.mul(10).add(time.mul(2))).mul(0.5).add(0.5)
                
                // Gradient from primary to secondary color
                const primaryColor = color(colors.primary)
                const secondaryColor = color(colors.secondary)
                
                // Mix based on UV and animated wave
                const gradientMix = baseUv.y.add(wave.mul(0.3))
                const bannerColor = mix(primaryColor, secondaryColor, gradientMix)
                
                // Add shimmer effect
                const shimmer = sin(baseUv.x.mul(20).add(baseUv.y.mul(10)).add(time.mul(5))).mul(0.1).add(0.9)
                
                return vec4(bannerColor.mul(shimmer).mul(1.2), float(1))
            })()
    ...
    setLeaderboard()
    {
        ...
        this.leaderboard.videoMode = true // Toggle between video and leaderboard display
        const resolution = 512

        // Create video element for the display
        ...
        this.leaderboard.videoTexture = videoTexture

        // Canvas for fallback/text overlay
        ...
        // Create material with video display
        ...
        const videoModeUniform = uniform(1) // 1 = video mode, 0 = leaderboard mode
        this.leaderboard.videoModeUniform = videoModeUniform
        ...
        material.outputNode = Fn(() =>
        {
            ...
            // Video content
            const videoColor = texture(videoTexture, baseUv)
            
            // Text/leaderboard content
            const textContent = texture(textTexture, baseUv)
            
            // Mix between video and text based on mode
            const displayColor = mix(
                mix(baseOutput.rgb, textContent.rgb.mul(1.3), textContent.a),
                videoColor.rgb.mul(1.2),
                videoModeUniform
            )
            
            // Add subtle scanline effect for retro TV look
            const scanline = sin(baseUv.y.mul(resolution).mul(2)).mul(0.03).add(0.97)
            
            // Add vignette effect
            const vignette = float(1).sub(baseUv.sub(0.5).length().mul(0.5))
            
            return vec4(displayColor.mul(scanline).mul(vignette), baseOutput.a)
        })()
        ...
        // Method to set custom video URL
        this.leaderboard.setVideoUrl = (url) => {
            video.src = url
            video.load()
            startVideo()
        }
        
        // Toggle between video and leaderboard mode
        this.leaderboard.toggleMode = () => {
            this.leaderboard.videoMode = !this.leaderboard.videoMode
            videoModeUniform.value = this.leaderboard.videoMode ? 1 : 0
            
            if(this.leaderboard.videoMode) {
                startVideo()
            } else {
                video.pause()
            }
    ...
    setMenu()
    {
        ...
        {
            // Always load and display local scores when menu opens
            const scores = this.localScores ? this.localScores.load() : []
                    ...
                    for(const score of scores)
                    {
                        html += /* html */`
                            <tr>
                                <td>${rank}</td>
                                <td>${score.tag || 'YOU'}</td>
                                <td>${timeToRaceString(score.time / 1000)}</td>
                            </tr>
                        `
    ...
    setEndModal()
    {
        ...
        const submit = () =>
        {
            ...
            // Save score locally (no server needed)
            if(sanatized.length === 3)
            {
                // Add to local scores
                const scores = this.localScores.add(this.timer.elapsedTime, sanatized)
        ...
        const updateGroup = () =>
        {
            // Always allow submission if tag is 3 characters (no server check)
    ...
    setData()
    {
        // Local score storage - no server needed
        this.localScores = {}
        this.localScores.load = () =>
        {
            const stored = localStorage.getItem('circuitScores')
            if(stored)
            {
                try {
        ...
        this.localScores.save = (scores) =>
        {
            // Keep only top 10 scores
            const topScores = scores.slice(0, 10)
            localStorage.setItem('circuitScores', JSON.stringify(topScores))
        }
        
        this.localScores.add = (time, tag = 'YOU') =>
        {
            const scores = this.localScores.load()
            const newScore = {
                tag: tag,
        ...
        // Don't update leaderboard here - menu isn't initialized yet
        // It will be loaded when menu opens (handled in setMenu)
    }
    ...
    finish(forced = false)
    {
        ...
        gsap.delayedCall(forced ? 1 : 4, () =>
        {
            ...
            this.game.overlay.show(() =>
            {
                ...
                // Show end modal with score (always available - no server needed)
                if(!forced)
                {
                    gsap.delayedCall(1, () =>
                    {
                        // Check if this is a top 10 time
                        const scores = this.localScores.load()
                        const timeMs = this.timer.elapsedTime * 1000
                        const isTop10 = scores.length < 10 || timeMs < (scores[scores.length - 1]?.time || Infinity)
