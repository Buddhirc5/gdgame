...
export class Menu
{
    ...
    setTrigger()
    {
        ...
        // Add C key to toggle menu
        this.game.inputs.addActions([
            { name: 'menuToggle', categories: [ 'wandering', 'racing' ], keys: [ 'Keyboard.KeyC' ] }
        ])
        
        this.game.inputs.events.on('menuToggle', (action) =>
        {
            if(action.active)
            {
                if(this.state === Menu.OPEN || this.state === Menu.OPENING)
                {
                    this.close()
                }
                else
