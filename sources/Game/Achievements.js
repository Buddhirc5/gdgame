...
export class Achievements
{
    constructor()
    {
        ...
        // Unlock ALL achievements by default (offline mode - everything unlocked)
        this.unlockAll()
    ...
    // Unlock all achievements and rewards
    unlockAll()
    {
        // Set all achievements to completed
        this.groups.forEach((group, name) =>
        {
            // Get max progress needed for this group
            let maxProgress = 0
            for(const item of group.items)
            {
                if(item.total > maxProgress)
                    maxProgress = item.total
            }

            // If it's a Set-based progress (unique achievements)
            if(group.progress instanceof Set)
            {
                // Add enough items to complete
                for(let i = 0; i < maxProgress; i++)
                {
                    group.progress.add(`unlocked_${i}`)
                }
            }
            else
            {
                group.progress = maxProgress
            }
            ...
            for(const achievement of group.items)
            {
                ...
                if(achievement.progressCurrentElement)
                    achievement.progressCurrentElement.textContent = achievement.total
                if(achievement.barFillElement)
                    achievement.barFillElement.style.transform = 'scaleX(1)'
            }
        })
    ...
    setRewards()
    {
        ...
        // Update - All rewards are unlocked by default (offline mode)
        this.rewards.update = () =>
        {
            this.rewards.items.forEach(item =>
            {
                // Always unlock all rewards (offline mode - everything available)
                if(item.locked)
                {
                    item.locked = false
